...
const LocationModal = () => {
  const { selectedLocation, locationModalOpen, setLocationModalOpen, updateLocation, detectGPSLocation } = useLocation();
  const { user } = useAuth();
  ...
  const handlePincodeCheck = (e) => {
    ...
    if (code.length === 6 && (code.startsWith('11') || code.startsWith('12') || code.startsWith('40') || code.startsWith('56') || code.startsWith('70') || code.startsWith('60'))) {
      setPincodeStatus({ serviceable: true, message: '🎉 Superfast 12-min delivery is available in your area!' });
      updateLocation({
        ...selectedLocation,
        pincode: code,
        city: code.startsWith('11') ? 'New Delhi' : code.startsWith('12') ? 'Gurugram' : code.startsWith('40') ? 'Mumbai' : 'Bengaluru',
        streetAddress: `Area Pincode ${code}`,
      });
    } else {
      setPincodeStatus({ serviceable: false, message: 'Sorry, we currently deliver to select metros (Delhi-NCR, Mumbai, Bengaluru).' });
    }
  };
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
          <div>
            ...
            <p style={{ fontSize: '0.85rem', color: '#64748b' }}>We deliver instant groceries in 10–30 minutes</p>
          </div>
          <button
        ...
        <button
          ...
          style={{ marginBottom: '20px', padding: '12px 18px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
        >
          <Navigation size={18} />
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', maxHeight: '220px', overflowY: 'auto' }}>
              {savedAddresses.map((addr) => {
                const isCurrent = selectedLocation?.id === addr.id || selectedLocation?.streetAddress === addr.streetAddress;
                return (
                  <div
                    key={addr.id}
                  ...
                  >
                    <div style={{
                      ...
                      {addr.label?.toLowerCase() === 'home' ? <Home size={18} /> : addr.label?.toLowerCase() === 'work' ? <Briefcase size={18} /> : <Building size={18} />}
                    </div>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <span style={{ fontWeight: '700', fontSize: '0.9rem', color: '#0f172a' }}>{addr.label}</span>
                        {isCurrent && <Check size={16} color="#059669" />}
                      </div>
                      <p style={{ fontSize: '0.82rem', color: '#64748b', marginTop: '2px', lineHeight: '1.3' }}>
